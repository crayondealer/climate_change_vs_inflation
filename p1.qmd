...
```{r}
ghg_long |>
  filter(Series %in% c("CO2 emissions from gaseous fuel consumption (kt)", "CO2 emissions from liquid fuel consumption (kt)", "CO2 emissions from solid fuel consumption (kt)") & group!="Other") |>
  ggplot()+
  ...
  labs(fill="Reasons",
       y="GHG Emission (billion tonnes)",
       title="Major reasons for Greenhouse gas Emissions")  +
  scale_x_continuous(breaks = seq(1990, 2020, by = 10), limits = c(1990,2016), expand = c(0, NA)) +
  scale_y_continuous(breaks = seq(0, 8, by = 2), limits = c(0,10), expand = c(0, NA)) +
  scale_color_manual(labels=c('Gaseous fuel', 'Liquid fuel', 'Solid fuel'),
                     values=c("peachpuff2", "salmon2", "salmon4"))+
  scale_fill_manual(labels=c('Gaseous fuel', 'Liquid fuel', 'Solid fuel'),
                    values=c("peachpuff2", "salmon2", "salmon4"))
```
