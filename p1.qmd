...
ghg_types_long |>
  ggplot() +
  aes(x = Year) +
  geom_area(aes(y = Value, fill = factor(Series)), alpha = 0.7, linewidth = 1, colour = "white") +
  labs(y = 'Greenhouse gas emissions (Bt)',
       title = "Causes for Global Warming",
       caption = "*Greenhouse gas emissions are in Billion Tonnes of CO2 equivalent") +
  theme_minimal() +
  ...
  geom_line(data = forest_long,
            aes(x = as.numeric(Year), y = Value),
            linewidth = 1, colour = "forestgreen", linetype = "dashed") +
  geom_text(data = subset(forest_long, Year == max(Year)),
            aes(label = "Forest Area", color = Series, x = Inf, y = Value),
            vjust = 1.5, hjust = 1.1) +
  scale_color_manual(labels = c("Forest Green"), values=c("forestgreen"), guide="none") 
...
ghg_long |>
  ...
  labs(title="Sector-wise: GHG Emission in Percentage")  +
...
ghg_long |>
  ...
  labs(x='GHG Emission (billion tonnes)',
    ...
    caption = "*GHG Emissions in the last decade"
  )  +
...
joined_dataset_byyear |>
  ...
  geom_smooth(method = "lm", size=1, colour="navy", alpha=0.4) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
    plot.margin = unit(c(0.1, 0.5, 0.1, 0.5), "inches")
